...
gulp.task('sass:includes', function() {
  ...
  return plugins
    .file('src/app/_includes.scss', str, { src: true })
    .pipe(plugins.plumber())
    .pipe(gulp.dest('.'));
});
...
gulp.task('inject:sass', function() {
  return gulp
    .src('src/app/_includes.scss')
    .pipe(plugins.plumber())
    .pipe(
      plugins.inject(gulp.src(['src/{app,views,styles}/**/*.scss', '!src/app/app.scss', '!src/app/_includes.scss', '!src/styles/bootstrap-override.scss', '!src/styles/variables.scss'], { read: false }), {
        addRootSlash: false,
        relative: true,
...
gulp.task('sass', ['inject:sass'], function() {
  return gulp
    ...
    .pipe(plugins.plumber())
    .pipe(plugins.sass({ precision: 10, outputStyle: 'nested', sourceComments: true }).on('error', plugins.sass.logError))
...
gulp.task('vendors', function() {
  return (
    gulp
      .src(['src/vendors/jquery-ui.js', 'src/vendors/gridstack.js', 'src/vendors/gridstack-jqueryui.js', 'src/vendors/ui-bootstrap.js', 'src/vendors/ui-bootstrap-tpls.js', 'src/vendors/**/*.js'])
      .pipe(plugins.plumber())
      .pipe(plugins.concat('vendors.js', { newLine: ';' }))
      // .pipe(plugins.if(settings.isDistribution, plugins.babel()))
      .pipe(plugins.if(settings.isDistribution, plugins.uglify()))
      .pipe(plugins.if(settings.isDistribution, plugins.rev()))
      .pipe(gulp.dest('dist/js'))
...
gulp.task('inject:partials', ['partials'], function() {
  return gulp
    ...
    .pipe(plugins.plumber())
    .pipe(
      plugins.inject(gulp.src('dist/js/partials-**.js', { read: false }), {
...
gulp.task('inject:revisions', function() {
  return gulp
    .src(['dist/revisions/*.json', 'dist/index.html'])
    .pipe(plugins.plumber())
...
gulp.task('assets', ['clean:assets'], function() {
  return gulp
    .src('src/assets/{images,fonts,icons,misc}/**/*')
    .pipe(plugins.plumber())
    .pipe(gulp.dest('dist/'))
    .pipe(plugins.browserSync.stream());
});
...
gulp.task('watch', function() {
  ...
  plugins
    .watch(['src/{app,views,styles}/**/*.scss'])
    .on('change', sass)
    .on('add', sass)
    .on('unlink', sass);
...
gulp.task('build', function() {
  settings.env = 'dist';
  plugins.runSequence('clean', 'bower-install', 'sass:includes', 'vendors', 'scripts', 'sass', 'assets', 'bower-fonts', 'inject:bower', 'move-index', 'useref', 'inject:partials', 'inject:revisions', 'clean:revisions', function() {
    settings.env = 'dev';
  });
});
...
gulp.task('build:serve', function() {
  settings.env = 'dist';
  plugins.runSequence('clean', 'bower-install', 'sass:includes', 'vendors', 'scripts', 'sass', 'assets', 'bower-fonts', 'inject:bower', 'move-index', 'useref', 'inject:partials', 'inject:revisions', 'clean:revisions', 'connect:build', function() {
    settings.env = 'dev';
  });
});
